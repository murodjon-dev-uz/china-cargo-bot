...
// The "Контакты" and "Менеджеры" sheets are the access lists: a phone must be
// on one of them for its owner to use the bot at all, and which one decides
// the role. Apps Script sends the ENTIRE tab on any edit, and we reconcile
// wholesale within that role — numbers missing from the payload are deleted,
// which is what revokes access on the next message.
app.post('/webhook/contacts-sync', async (req, res) => {
  ...
  try {
    ...
    const result = await queries.withTransaction((client) => queries.replaceContacts(
      rows.map((r) => ({ fullName: r.name || null, phone: r.phone, sheetRow: r.row || null })),
      role,
      client
    ));
    logger.info('webhook: contacts synced', role, `${result.kept} allowed`, `${result.removed} revoked`);
