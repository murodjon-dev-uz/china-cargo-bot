 ...
 */
function paymentLines(order, { enabled = config.paymentsEnabled } = {}) {
  const price = parseDecimal(order.price);
  // Payments hidden: the price is still cargo information the client asked
  // for, so it stays — only the ledger-derived lines go.
  if (!enabled) return price === null ? [] : [`Стоимость: <b>${escapeHtml(formatMoney(price, order.currency))}</b>`];
